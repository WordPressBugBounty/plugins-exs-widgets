...
$image_size = ! empty ( $exs_image_size ) ? $exs_image_size : 'large';

echo wp_kses_post( str_replace( 'class="', 'class="widget-fullwidth ', $exs_args['before_widget'] ) );
echo '<div class="widget-posts-gird grid-absolute layout-cols-' . esc_attr( $exs_columns . ' layout-' . $exs_layout . ' layout-gap-' . $exs_gap . ' ' . $exs_css_class . $exs_center_class ) . '">';
if ( $exs_title ) {
...
?>
	<div class="grid-wrapper">
		...
		foreach ( $exs_r->posts as $exs_post ) :
			...
			$exs_post_thumbnail = get_the_post_thumbnail( $exs_post->ID, $image_size );
			$exs_post_class     = ( ! empty( $exs_post_thumbnail ) ) ? 'content-absolute' : '';
			?>
			<div class="grid-item">
				<article <?php post_class( '', $exs_post->ID ); ?>>
					<?php if ( ! empty( $exs_post_thumbnail ) ) : ?>
						<div class="content-absolute">
							<div class="posts-list-thumbnail">
								...
								?>
								<a class="abs-link" href="<?php the_permalink( $exs_post->ID ); ?>"></a>
							</div>
