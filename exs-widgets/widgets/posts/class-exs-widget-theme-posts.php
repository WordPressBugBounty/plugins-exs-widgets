...
if ( ! class_exists( 'ExS_Widget_Theme_Posts' ) ) :
	...
	class ExS_Widget_Theme_Posts extends WP_Widget {
		...
		public function widget( $exs_args, $exs_instance ) {
			...
			$exs_image_size   = ( ! empty( $exs_instance['image_size'] ) ) ? esc_attr( $exs_instance['image_size'] ) : '';
			$exs_read_more    = ( ! empty( $exs_instance['read_more'] ) ) ? sanitize_text_field( $exs_instance['read_more'] ) : '';
			...
			if ( file_exists( $exs_filepath ) && in_array( $exs_layout, $exs_layouts_array ) ) {
		...
		public function update( $exs_new_instance, $exs_old_instance ) {
			...
			$exs_instance['image_size']  = esc_attr( $exs_new_instance['image_size'] );
			$exs_instance['read_more']    = sanitize_text_field( $exs_new_instance['read_more'] );
		...
		public function form( $exs_instance ) {
			...
			$exs_image_size  = isset( $exs_instance['image_size'] ) ? esc_attr( $exs_instance['image_size'] ) : '';
			$exs_read_more   = isset( $exs_instance['read_more'] ) ? sanitize_text_field( $exs_instance['read_more'] ) : '';
			...
			<p>
				<label
					for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php esc_html_e( 'Grid layouts image size', 'exs' ); ?></label>
				<select
					name="<?php echo esc_attr( $this->get_field_name( 'image_size' ) ); ?>"
					id="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"
					class="widefat"
				>
					<option value=""<?php selected( $exs_image_size, '' ); ?>><?php esc_html_e( 'Default', 'exs' ); ?></option>
					<?php
					$image_sizes = get_intermediate_image_sizes();

					foreach ( $image_sizes as $image_size ) {
						?>
						<option value="<?php echo esc_attr( $image_size ); ?>"<?php selected( $image_size, $exs_image_size ); ?>><?php echo esc_html( $image_size ); ?></option>
						<?php
					}
					?>
				</select>
			...
			<p>
				<label
						for="<?php echo esc_attr( $this->get_field_id( 'read_more' ) ); ?>"><?php esc_html_e( '\'Read More\' post link text', 'exs' ); ?></label>
				<input
						class="widefat"
						id="<?php echo esc_attr( $this->get_field_id( 'read_more' ) ); ?>"
						name="<?php echo esc_attr( $this->get_field_name( 'read_more' ) ); ?>"
						type="text"
						value="<?php echo esc_attr( $exs_read_more ); ?>"/>
			</p>
			...
			?>
				<div class="cats-links cats-<?php echo esc_attr( $layout ); ?>">
					<span class="icon-inline post-date">
						...
						<?php echo wp_kses_post( get_the_category_list( '<span class="cats-separator">,&nbsp;</span>', '', $post_id ) ); ?>
					</span>
				</div>
			<?php
				break;
