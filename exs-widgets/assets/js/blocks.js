( function( blocks, element, serverSideRender, components, blockEditor, i18n , compose, data) {
	...
	var isResolving = select('core');
	if(isResolving) {
		isResolving = isResolving.getEntityRecords('taxonomy', 'category',{'per_page': -1});
	}
	...
	registerBlockType( 'exs-blocks/exs-widget-posts', {
			...
			edit: function( props ) {
				...
				//new image sizes
				var imageSizesBlank = [{value: '', label: __('Default', 'exs')}];
				var imageSizeOptions = imageSizesBlank;
				var allImageSizes = useSelect(function(sel) {
					if(isResolving) {
						return imageSizesBlank;
					} else {
						var isResolved = sel('core/block-editor').getSettings().imageSizes;
						if(isResolved){
							var allImageSizes = isResolved.map(function (cat,i){
								return {value:cat.slug,label:cat.name};
							});
							imageSizeOptions = allImageSizes.concat(imageSizesBlank);
					...
					el(
						...
						el(
							...
							el(
								SelectControl,
								{
									label: __('Grid layouts image size', 'exs'),
									value: props.attributes.image_size,
									options: allImageSizes,
									onChange: function (val) {
										return props.setAttributes(
											{
												image_size: val
							...
							el(
								TextControl,
								{
									label: __('"Read More" link text', 'exs'),
									value: props.attributes.read_more,
									onChange: function (val) {
										return props.setAttributes(
											{
												read_more: val
							...
							el(
								TextControl,
								{
									label: __('"Read All" link text', 'exs'),
